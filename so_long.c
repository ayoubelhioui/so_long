...
/*   Updated: 2021/12/18 18:43:50 by ael-hiou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_move_x(t_mlx_lst *list, int collectible_count, \
		int direction, int *movescounter)
{
	ft_check_enemy_facing(list, direction, 'x');
	if (!((list->string[list->playerx + direction][list->playery] == 'E') \
				&& (collectible_count > 0)))
	{
		if (!(list->string[list->playerx + direction][list->playery] == '1'))
		{
			if ((collectible_count == 0) && (list->string[list->playerx \
						+ direction][list->playery] == 'E'))
				exit(1);
			if (list->string[list->playerx + direction][list->playery] == 'C')
				list->string[list->playerx + direction][list->playery] = '0';
			*movescounter += 1;
			ft_create_elements(list, (list->playery) * MAP_UNITY, \
					(list->playerx * MAP_UNITY), BLACK_IMG);
			list->playerx += direction;
			ft_create_elements(list, (list->playery) * MAP_UNITY, \
					(list->playerx) * MAP_UNITY, PLAYER_IMG);
		}
	}
}

void	ft_move_y(t_mlx_lst *list, int collectible_count, \
		int direction, int *movescounter)
{
	ft_check_enemy_facing(list, direction, 'y');
	if (!((list->string[list->playerx][list->playery + direction] == 'E') \
				&& (collectible_count > 0)))
	{
		if (!(list->string[list->playerx][list->playery + direction] == '1'))
		{
			if ((collectible_count == 0) && (list->string[list->playerx] \
						[list->playery + direction] == 'E'))
				exit(1);
	...
	while (list->string[++i])
	{
		...
		while (list->string[i][++j])
		{
			if (list->string[i][j] == '1')
				...
				ft_create_elements(list, j * MAP_UNITY, i * \
						MAP_UNITY, COLLECTIBLE_IMG);
			else if (list->string[i][j] == 'E')
				ft_create_elements(list, j * MAP_UNITY, i \
						* MAP_UNITY, EXIT_IMG);
			else if (list->string[i][j] == 'N')
				ft_create_elements(list, j * MAP_UNITY, \
						i * MAP_UNITY, ENEMY_IMG);
		}
	}
}
...
void	so_long(int fd)
{
	...
	list.mlx = mlx_init();
	list.win = mlx_new_window(list.mlx, (var.window_width * MAP_UNITY), \
			(var.window_height * MAP_UNITY), "so_long");
